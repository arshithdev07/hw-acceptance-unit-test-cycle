  ...
  describe '.find_similar_movies' do
    ...
    context 'director exists' do
      it 'finds similar movies correctly' do
        expect(Movie.same_directors(movie1.director).size).to eql(2)
        expect(Movie.same_directors(movie2.director).size).to eql(1)
      end
    end

    context 'director does not exist' do
      it 'handles sad path' do
        expect(Movie.same_directors(movie4.title).size).to eql(0)
      end
    end
  end
